...
- `mapCoordinateScale = max(1, max(pixelW/170, pixelH/85))`; `mapNorth = pixelH/scale (~120.2)`, `mapEast = pixelW/scale (~170.0)`; Y flipped (`pixelYToMapLatitude(y) = (pixelBounds.north − y)/scale`). *(verified exact vs `campus_overlay_meta.dart:41-70`.)*
