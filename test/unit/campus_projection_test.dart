...
void main() {
  test('geometry wrappers hold their value without conflation', () {
    expect(const GpsPoint(LatLng(-33.77, 151.11)).value.latitude, -33.77);
    expect(const CampusMapPoint(LatLng(42.5, 60.1)).value.longitude, 60.1);
  });
  ...
  test('reference vector: affine matches MQ published coefficients', () {
    ...
    const nLng = (151.1134 - minLng) / (maxLng - minLng);
    const nLat = (-33.7737 - minLat) / (maxLat - minLat);
    const x = 880.374832 + 3889.927306 * nLng + 12.547607 * nLat;
    const y = 2862.069113 - 64.093654 * nLng - 2349.078357 * nLat;
  ...
  test('invalid GPS → null (NaN / out of range)', () {
    expect(_proj.project(const GpsPoint(LatLng(double.nan, 151.11))), isNull);
