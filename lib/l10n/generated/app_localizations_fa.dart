...
class AonL10nFa extends AonL10n {
  ...
  String get settingsPrivacyBody =>
      'برنامه حساب کاربری یا ورود ندارد. تبلیغ، تحلیل تحت مدیریت توسعه‌دهنده، ردیابی میان برنامه‌ها یا فروش دادهٔ شخصی وجود ندارد. شب من، پیشرفت گذرنامه، علاقه‌مندی‌ها، تنظیمات و رضایت Google Maps به‌صورت محلی در localStorage مرورگر در وب یا حافظهٔ محلی برنامه در iOS و Android ذخیره می‌شوند؛ نسخه‌های پشتیبان دستگاه یا سیستم موبایل ممکن است شامل دادهٔ محلی باشند.\n\nنسخه‌های بومی iOS و Android می‌توانند برای اسکن QR از دوربین استفاده کنند؛ وب از کد دستی استفاده می‌کند. تصاویر دوربین توسط برنامه ذخیره یا بارگذاری نمی‌شوند. فقط در Android، Google ML Kit اسکن QR را پردازش می‌کند و گزارش‌های خدماتی و شناسه‌های هر نصب را برای گوگل می‌فرستد.\n\nموقعیت اختیاری است و به‌صورت محلی برای امکانات پردیس و نقشه استفاده می‌شود. اگر پس از ثبت رضایت از مسیر پیاده‌روی گوگل استفاده کنید، ممکن است موقعیت دقیق یا تقریبی به‌عنوان مبدأ مسیر برای گوگل ارسال شود.\n\nجزئیات کامل، نگهداری، حذف و اطلاعات تماس را در سیاست حریم خصوصی زیر بخوانید.';
  ...
  String webPrivacyIntro(String developer, String forTeam) {
    return 'Astronomy Open Night 2026 توسط $developer برای $forTeam توسعه یافته است.';
  }

  @override
  String get webPrivacyStorageHeading => 'داده‌های ذخیره‌شده به‌صورت محلی';

  @override
  String get webPrivacyStorageBody =>
      'شب من و علاقه‌مندی‌ها، پیشرفت گذرنامهٔ نجوم، تنظیمات از جمله زبان و ظاهر، و وضعیت رضایت نقشهٔ گوگل به‌صورت محلی روی دستگاه شما ذخیره می‌شوند. نسخهٔ وب از localStorage مرورگر استفاده می‌کند و نسخه‌های iOS و Android از حافظهٔ محلی برنامه. ما سروری برای نگهداری یا دسترسی به این داده‌های محلیِ تحت مدیریت برنامه نداریم. در iOS و Android، بسته به تنظیمات دستگاه و پشتیبان‌گیری، نسخه‌های پشتیبان دستگاه یا سیستم ممکن است شامل داده‌های محلی برنامه باشند.';
  ...
  String get webPrivacyLocationBody =>
      'دسترسی به موقعیت مکانی اختیاری است. اگر اجازه دهید، موقعیت شما به‌صورت محلی برای امکانات پردیس و نقشه، مانند نمایش جای شما و در نسخه‌های iOS و Android برای جهت‌دهی قطب‌نما به‌سوی یک مکان، استفاده می‌شود. برنامه موقعیت شما را ذخیره نمی‌کند یا برای توسعه‌دهندگان نمی‌فرستد. وقتی مسیر پیاده‌روی را با Google Maps درخواست می‌کنید، درخواست مسیر ارسالی به گوگل ممکن است شامل موقعیت دقیق یا تقریبی شما به‌عنوان مبدأ و مقصد انتخاب‌شده باشد. اگر موقعیت را مجاز نکنید، نقشهٔ پردیس همچنان در دسترس است.';
  ...
  String get webPrivacyCameraBody =>
      'در نسخه‌های بومی iOS و Android می‌توانید برای اسکن کدهای QR گذرنامهٔ نجوم از دوربین استفاده کنید. تصاویر دوربین برای اسکن پردازش می‌شوند و برنامه آن‌ها را ذخیره یا بارگذاری نمی‌کند. ورود دستی کد ایستگاه نیز در دسترس است. نسخهٔ وب از ورود دستی کد ایستگاه استفاده می‌کند و به اسکن دوربین متکی نیست.';

  @override
  String get webPrivacyMlKitHeading => 'فقط Android: Google ML Kit';

  @override
  String get webPrivacyMlKitBody =>
      'نسخهٔ Android هنگام استفاده از اسکنر QR از بارکدخوان Google ML Kit استفاده می‌کند. گوگل اعلام می‌کند که SDK اندروید ML Kit اطلاعات دستگاه و برنامه، شناسه‌های هر نصب، معیارهای عملکرد، پیکربندی API، اطلاعات رویداد ویژگی و کدهای خطا را برای عیب‌یابی و تحلیل استفاده جمع‌آوری می‌کند و این داده‌ها هنگام انتقال با HTTPS رمزگذاری می‌شوند. این توضیح ML Kit شامل نسخه‌های iOS یا وب نمی‌شود.';
  ...
  String get webPrivacyMapsBody =>
      'پردازش Google Maps و Google Routes فقط هنگامی رخ می‌دهد که از قابلیت مرتبطِ نقشه یا مسیر پیاده‌روی گوگل استفاده کنید و رضایت لازم ثبت شده باشد. بسته به پلتفرم و قابلیت، گوگل ممکن است درخواست نقشه یا مسیر، مبدأ و مقصد مسیر، نشانی IP، اطلاعات دستگاه، برنامه، مرورگر و درخواست، شناسهٔ مستعار مخصوص Maps SDK، گزارش‌های خرابی و تعامل با نقشه مانند جابه‌جایی و بزرگ‌نمایی را پردازش کند. گوگل این اطلاعات را طبق سیاست حریم خصوصی خود (https://policies.google.com/privacy) پردازش می‌کند. می‌توانید انتخاب رضایت Google Maps را در تنظیمات تغییر دهید.';

  @override
  String get webPrivacyAnalyticsHeading => 'حساب، تبلیغات، تحلیل و ردیابی';

  @override
  String get webPrivacyAnalyticsBody =>
      'برنامه‌ها حساب کاربری یا ورود ندارند. ما تبلیغ نمایش نمی‌دهیم، سرویس تحلیل خودمان را اجرا نمی‌کنیم، از پیکسل ردیابی استفاده نمی‌کنیم، شما را میان برنامه‌ها یا وب‌سایت‌های دیگر ردیابی نمی‌کنیم و داده‌های شخصی شما را نمی‌فروشیم. هنگام استفاده از قابلیت‌های Google Maps، Routes یا Android ML Kit که بالا توضیح داده شد، گوگل ممکن است داده‌های خدماتی و تشخیصی را پردازش کند؛ این پردازش سرویس تحلیل تحت مدیریت توسعه‌دهندگان نیست.';
  ...
  String get webPrivacyRetentionBody =>
      'داده‌های محلیِ تحت مدیریت برنامه تا زمان حذف روی دستگاه شما می‌مانند. تنظیمات ← حذف داده‌های من، شب من، پیشرفت گذرنامهٔ نجوم، علاقه‌مندی‌ها و وضعیت رضایت نقشهٔ گوگل را پاک می‌کند و عمداً زبان و تنظیمات ظاهری را نگه می‌دارد. حذف داده‌های محلی برنامه، اطلاعاتی را که قبلاً گوگل پردازش کرده یا نسخه‌هایی را که در پشتیبان‌های دستگاه یا سیستم نگهداری می‌شوند خودکار حذف نمی‌کند. نگهداری و حذف اطلاعات پردازش‌شده توسط خدمات گوگل تابع سیاست‌های خود گوگل است.';

  @override
  String get webPrivacyContactHeading => 'تماس و پشتیبانی';

  @override
  String webPrivacyContactBody(
    String privacyEmail,
    String supportEmail,
    String eventWebsite,
    String privacyUrl,
  ) {
    return 'پرسش‌های حریم خصوصی و توسعه‌دهنده: $privacyEmail. پرسش‌های رویداد و پشتیبانی: $supportEmail. اطلاعات رسمی رویداد: $eventWebsite. سیاست حریم خصوصی مرجع در $privacyUrl در دسترس است.';
  }

  @override
  String webPrivacyCredit(String developer, String forTeam) {
    return 'توسعه‌یافته توسط $developer برای $forTeam.';
  }
  ...
  String get webPrivacyScope =>
      'این سیاست حریم خصوصی به‌طور مشخص برای برنامه‌های موبایل iOS و Android و برنامهٔ وب Astronomy Open Night 2026 است و شامل هیچ محصول، برنامه یا وب‌سایت نامرتبطی نمی‌شود.';
