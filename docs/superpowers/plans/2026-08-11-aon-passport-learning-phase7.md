...
```bash
git branch --show-current   # expect feature/passport-learning-phase7
...
- [ ] **Step 3: Baseline builds** — Run: `flutter build web && flutter build apk --debug` — Expected: both succeed (no new deps this phase, so this should stay green throughout).
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          // Activity context (heading semantics).
          Semantics(
            header: true,
            child: Text(fact.activityLabel.toUpperCase(),
...
// Counts HapticFeedback platform calls.
int _hapticCalls = 0;
...
void main() {
  testWidgets('a 1st collect opens the fact sheet and fires one haptic',
      (t) async {
    _installHapticSpy();
  ...
  testWidgets('the 9th collect goes to reward, not the fact sheet', (t) async {
    ...
    expect(_hapticCalls, 1); // haptic still fires once on the 9th
  });
  ...
  Widget build(BuildContext context) {
    ...
    if (collected) {
      return Semantics(
        ...
        onTap: () => onTapCollected?.call(venueId),
...
- [ ] **Step 3: iOS runtime pass** (simulator; collect via manual entry — no camera needed). Record PASS/FAIL for each:
```text
[ ] collect a non-final stamp (manual code) → exactly one haptic (feel/observe)
[ ] fact sheet opens for the correct venue, restrained stamp pop
[ ] close sheet → capture flow continues (Scan another / manual still work)
[ ] revisit the fact by tapping the collected grid cell
...
```
(Release-safe fallback is proven by the `320×568/2.0` + publication-policy tests;
a physical device is not required — no camera path is exercised here.)

- [ ] **Step 4: Content closeout** — record in the spec's §17/closeout: confirmed N/9, derived N/9, placeholder N/9 (expected **0/0/9** at first ship — all drafts await sign-off), and `sourceRegistry` complete PASS.
...
git commit -m "docs(passport): Phase 7 verification + content closeout (0/0/9 drafts pending review)"
```
...
- **Never weaken a Phase 1–6 test.** The Phase 6 grid/screen/scan tests must stay green as-is; if one breaks, the integration is wrong.
