...
void main() {
  test('parses nodes and builds an equirectangular Pannellum config', () {
    ...
    final cfg = m.buildPannellumConfig(
      assetBaseUrl: 'http://localhost:8459/data',
    );
    ...
    expect(
      (scenes['a'] as Map)['panorama'],
      'http://localhost:8459/data/indoor/a.jpg',
    );
  ...
  test('SECURITY: unsafe image refs are blanked, never off-origin', () {
    for (final bad in [
      ...
      final m = IndoorManifest.fromJson(
        '{"nodes":[{"id":"a","image":${_json(bad)},"neighbours":[]}]}',
      );
      final scenes =
          m.buildPannellumConfig(
                assetBaseUrl: 'http://localhost:8459/data',
              )['scenes']
              as Map;
      expect((scenes['a'] as Map)['panorama'], '', reason: bad);
    }
    final ok = IndoorManifest.fromJson(
      '{"nodes":[{"id":"a","image":"indoor/a.jpg","neighbours":[]}]}',
    );
    final okScenes =
        ok.buildPannellumConfig(
              assetBaseUrl: 'http://localhost:8459/data',
            )['scenes']
            as Map;
    expect(
      (okScenes['a'] as Map)['panorama'],
      'http://localhost:8459/data/indoor/a.jpg',
    );
  });
  ...
  test('REDUCED MOTION: no scene fade and autoRotate off', () {
    ...
    final reduced =
        m.buildPannellumConfig(assetBaseUrl: 'x', reduceMotion: true)['default']
            as Map;
