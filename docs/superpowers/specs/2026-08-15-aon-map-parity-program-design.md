...
MarkerLayer             (M1: AON venues/parking re-seated via projection → M3: + MQ registry, hybrid)
...
The reskin is **generated by code, not hand-drawn**, so it is reproducible and re-runnable when MQ updates the art. Chosen direction **(c)**: map the low-saturation cream ground → dark AON slate scaled by luminance, **preserve saturated ink** (roads/greens/feature colours), dim the rest. The **same transform** is applied to all four `overlay_*.png` so base and overlays stay coherent. **Caveat:** in the three-way prototype (c) read as the *busiest/least-legible* of the candidates (b, the luminance-invert, was cleaner). So **M0 carries a hard on-device night-legibility gate**: tune (c) against the AON slate palette; if it cannot be made legible at event-night brightness, fall back to (b). The gate is a merge condition, not a preference.
...
| **M1 — Basemap platform** | Port `CampusOverlayMeta` + `CampusProjection(Impl)` + `CrsSimple` `FlutterMap`; **replace** AON's WGS84 map (remove `DarkTileLayer`; `initialCenter`→map-unit centre `meta.centerLat/Lng`; `CameraConstraint`→map-unit bounds); **re-seat Phase A** dot + accuracy circle **and the existing venue/parking `MarkerLayer`** onto `gpsToMapPoint` (else the pins strand off-map — see §7); off-campus guard on raw GPS pre-projection | M0 assets |
...
| **M5 — AR / compass** | AR building picker + compass view (reuse Phase B heading); **extend the toggle to `MapMode { campusMap, panorama, ar }`** — AON already ships `panorama` (`map_mode_toggle.dart`); AR is *added*, panorama is **not** dropped (confirm at M5, §13 Q6) | camera permission |
...
- **Phase A (live location):** the dot + accuracy circle move from WGS84 `LatLng` to **map-unit** points via `CampusProjection.gpsToMapPoint`. MQ's `CampusMapLocationLayer` is the proven pattern (accuracy metres → map units via `pixelWidth / gpsLongitudeSpanMetres / scale`, capped). The `LocationService`/`LocationController` seam and providers are **unchanged**; only the render layer's coordinate mapping changes. The off-campus guard runs on **raw GPS pre-projection** (compare to campus GPS bounds before projecting).
- **The existing venue/parking markers (M1, not M3):** AON currently places them at raw WGS84 (`map_screen.dart:264` `LatLng(v.latitude!, v.longitude!)`). Under `CrsSimple` a WGS84 `LatLng` is read as a **map-unit** point (range 0–120 lat / 0–170 lng), so `-33.77` falls off the south edge and **every pin vanishes**. M1 **must** re-seat the current venue/parking markers through `gpsToMapPoint` (they become the curated event layer); M3 then adds the MQ registry on top. Deferring all markers to M3 would leave M1's merged map visibly broken — this is a hard M1 requirement, not an M3 one.
...
- **Hard floor:** the existing test suite stays green. Any phase that reddens Phase A/B tests does not merge. The projection port ships with its own pure-math tests (GPS→pixel→map round-trips against MQ's published affine). **The "527" figure is carried forward from Phase B's summary and is NOT verified on this base (5312b14) — re-baseline the exact count at M1 Task 0 and freeze it there; do not assert 527.**

## 8. Hybrid data reconciliation (M3 design seed)

MQ's `Building` (170, campus registry, GPS + `campusX/Y` pixel coords, rich taxonomy) and AON's `Venue`/`ParkingArea` (curated, event-scoped, `DataConfidence`, West-6-null) are different shapes. M3 unifies them **behind AON's data-seam** (screens read providers, never `data/*_data.dart`): the MQ registry drives browse/search markers; AON's curated venues render as the **highlighted event layer** on top (distinct marker treatment). Conflicts resolve in favour of AON's curated coordinate + confidence where a building is also an event venue. **Mixed placement precision (design constraint):** MQ buildings carry pixel-exact `campusX/Y` → `buildingPixelToMapPoint` (no affine error); AON venues carry only GPS → `gpsToMapPoint` (subject to the affine's residual). So the two marker sets can place with different precision on the same base — M3 must decide whether to keep AON venues on GPS-affine or backfill `campusX/Y` for them. Full design deferred to the M3 spec.
...
- **IOU-P5** — MQ's `offline_maps_service` (flutter_map_tile_caching, for WGS84 tiles) is **N/A** under a bundled `CrsSimple` raster and is **not ported**; the basemap ships in-bundle (inherently offline). Revisit only if M4 routing needs offline tiles.
- **IOU-P6** — AON venue markers place via GPS-affine (approximate) vs MQ buildings' pixel-exact `campusX/Y` (§8); M3 decides whether to backfill `campusX/Y` for AON venues.
...
5. **Reskin (c) tuning target** — exact AON slate tokens + per-overlay opacity (M0/M2); fall back to (b) if (c) fails the night-legibility gate (§5).
6. **Toggle target** — confirm `MapMode { campusMap, panorama, ar }` (keep the shipped 360° panorama and add AR) vs replacing panorama with AR (M5, §6).
