...
void main() {
  ...
  group('supported locales', () {
    test('exactly English and Persian are declared', () {
      ...
      expect(
        AonL10n.supportedLocales.map((l) => l.languageCode).toSet(),
        {'en', 'fa'},
      );
    });
    ...
    test('the Persian ARB has every key the English template has', () {
      ...
      expect(faKeys.difference(enKeys), isEmpty,
          reason: 'Persian has keys English does not');
      expect(enKeys.difference(faKeys), isEmpty,
  ...
  group('resolution', () {
    testWidgets('English renders English copy', (tester) async {
      await tester.pumpWidget(app(const MyNightScreen(), locale: const Locale('en')));
      await tester.pumpAndSettle();
      expect(find.text('My Night'), findsWidgets);
    });

    testWidgets('Persian renders Persian copy', (tester) async {
      await tester.pumpWidget(app(const MyNightScreen(), locale: const Locale('fa')));
      await tester.pumpAndSettle();
    ...
    testWidgets('an unsupported locale falls back to English, not a crash',
        (tester) async {
      await tester.pumpWidget(app(const MyNightScreen(), locale: const Locale('de')));
      await tester.pumpAndSettle();
  ...
  group('right-to-left', () {
    ...
    testWidgets('the back arrow mirrors in Persian', (tester) async {
      ...
      expect(Directionality.of(tester.element(find.byType(AppBar))),
          TextDirection.rtl);
    ...
    for (final (name, screen) in <(String, Widget)>[
      ...
      testWidgets('$name renders in Persian RTL without overflow',
        ...
        await tester.pumpAndSettle();
        expect(tester.takeException(), isNull, reason: '$name overflowed in fa');
      });
    }
  ...
  group('no hardcoded Astronomy strings in widgets', () {
    test('key user-facing copy is not literal in lib/screens or lib/widgets',
        () {
      ...
      expect(offenders, isEmpty,
          reason: 'use an ARB key instead of a literal:\n${offenders.join('\n')}');
    });
  });
}
