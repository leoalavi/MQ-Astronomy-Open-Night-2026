...
| **R4** | ~~Redistribution permission unresolved for **4 asset sets**~~ **NARROWED 2026-09-05** to one: the Home hero photograph | Blocks public release | Owner's attestation settles the three MQ sets (basemap artwork, `buildings.json`, the 39 360° images). The hero is Aleix Roig's — the app *credits* it, which is not a licence; `README.md` says so itself | Get the photographer's written permission for app distribution **and** store screenshots, or substitute a hero the project owns. `docs/release-blockers.md` B6 |
